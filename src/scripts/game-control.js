...
function initEquipamento() {
  ...
  $("#schoolbag .equip").on("click", function () {
    $("#schoolbag .whapper .modal-close").removeClass(
  ...
  if (arrMedalhasConquistadas.length > 0) {
    $("#medal")
      ...
      .attr(
        "aria-label",
        `Você conquistou ${
          arrMedalhasConquistadas.length <= 1 ? "a medalha" : "as medalhas"
        } ${arrMedalhasConquistadas.toString()}`
      );
  } else {
    $("#medal")
      .find(".box")
      .attr("aria-label", `Você ainda não conquistou medalhas.`);
  }
}
...
function controlSemente() {
  ...
  $("body").on("sementes", function (e, curr, value) {
    if (!scorm.loadObject("sem" + curr)) scorm.saveObject("sem" + curr, value);
